...
const ProductDetails = () => {
  ...
  const getSimilar = async (pid, category) => {
    try {
      const { data } = await axios.get(
        `${process.env.REACT_APP_API}/product/similarProd/${pid}/${category}`
      );
  ...
  const isRev = (product, userId) => {
    if (product.reviews && product.reviews.length > 0) {
      for (let i = 0; i < product.reviews.length; i++) {
        if (product.reviews[i].uid.toString() === userId) {
          k = i;
          return i; // User has already submitted a review, return the index
        }
      }
    }
    return -1; // User has not submitted a review
  };
  useEffect(() => {
    if (params?.pid) {
      getProd();
  ...
  const navigateToReviewPage = (productId, index) => {
    navigate("/review", { state: { pid: productId, i: index } });
  };
  const navigateToEditReviewPage = (productId, index) => {
    navigate("/editReview", { state: { pid: productId, i: index } });
  };
  useEffect(() => {
      ...
      <div className="row container">
            ...
            <>
              ...
              <div className="arrow-buttons">
                {currentIndex > 0 && (
                  ...
                  >
                    <img
                      src="https://w7.pngwing.com/pngs/518/133/png-transparent-left-arrow-button-direction-back-reverse-previous-free-vector-graphics-thumbnail.png"
        ...
        <div className="col-md-6">
          ...
          {product.stock > 0 ? (
            ...
            <h6 className="card-text" style={{ color: "red" }}>
              Out of Stock
            </h6>
          )}
          {auth.user.cart.length > 0 && productInCart(product._id) ? (
            <button
          ...
          {product.reviews && product.reviews.length > 0 ? (
            isRev(product, auth.user._id) !== -1 ? (
              <button
                className="btn btn-secondary ms-1"
                style={{ backgroundColor: "purple", margin: "10px" }}
                onClick={() => navigateToEditReviewPage(product._id, k)}
              >
                Edit Review
              </button>
            ) : (
              <button
                className="btn btn-secondary ms-1"
                style={{ backgroundColor: "purple", margin: "10px" }}
                onClick={() => navigateToReviewPage(product._id, -1)}
              >
                Add Review
              </button>
            )
            ...
            <button
              className="btn btn-secondary ms-1"
              style={{ backgroundColor: "purple", margin: "10px" }}
              onClick={() => navigateToReviewPage(product._id, k)}
            >
              Add Review
            </button>
          )}
          ...
          <div>
            {product.reviews.map((review) => (
              ...
              <div className="card-body">
                ...
                <p className="card-text">{Array.from({ length: 5 }, (_, index) => {
